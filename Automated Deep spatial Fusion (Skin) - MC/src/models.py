...
class PatchWiseModel(BaseModel):
    ...
    def train(self):
        self.network.train()
        print('Start training patch-wise network ResNet-18: {}\n'.format(time.strftime('%Y/%m/%d %H:%M')))
    ...
    def test(self, path, verbose=True):
        ...
        for index, (image, file_name) in enumerate(data_loader):
            ...
            #maj_prob = 3 - np.argmax(np.sum(np.eye(4)[np.array(output).reshape(-1)], axis=0)[::-1])
            prob = torch.exp(output.data)
            confidence = torch.max(torch.exp(output.data))
            confidence = np.round(confidence * 100, 2)

            res.append([prob, ntpath.basename(file_name[0]), confidence])
...
"""
    ...
    def output_prob(self, input_tensor):
        self.network.eval()
        res = self.network(Variable(input_tensor))
        return res.squeeze()
...
class ImageWiseModel(BaseModel):
    ...
    def test(self, path, verbose=True, ensemble= True):
        ...
        for index, (image, file_name) in enumerate(data_loader):
            ...
            patches = patches.view(n_bins, -1, 2)
    ...
    def _patch_loader(self, path, augment):
        ...
        if self.args.debug and augment and os.path.exists(images_path):
            ...
            for index, (images, labels) in enumerate(output_loader):
                ...
                res = self.patch_wise_model.output_prob(images.view((-1, 3, 112, 112)))
                res = res.view((bsize, -1, 2)).data.cpu().numpy()
