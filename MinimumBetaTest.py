...
def Two_Stage_Output (p1, threshold, mode = 'test'):
    ...
    file.write('Deep Spatial Fusion output:\n')
    file.write('--------------------------------------\n')
...
if args.network == '2':
    ...
    valid_Fold = 'F3'
    trials = [1,2,3,4,5]
    Optimizers = ['COBYLA', 'DualAnn', 'NSGA2']
    for op in range(len(Optimizers)):
        print (op)
        print('Optimizer: ', Optimizers[op])
        for i in trials:
            if op == 0:
                file = open(r'G:/Automated Deep spatial Fusion (BACH) - Entropy/ReportsT_'+Test_Fold+'/COBYLA/ExcImgs_T'+Test_Fold+'_V'+valid_Fold+'_COBYLA_trial'+str(i)+'.txt', 'w')
            elif op == 1:
                file = open(r'G:/Automated Deep spatial Fusion (BACH) - Entropy/ReportsT_'+Test_Fold+'/DualAnn/ExcImgs_T'+Test_Fold+'_V'+valid_Fold+'_DualAnn_trial'+str(i)+'.txt', 'w')
            elif op == 2:
                file = open(r'G:/Automated Deep spatial Fusion (BACH) - Entropy/ReportsT_'+Test_Fold+'/NSGA2/ExcImgs_T'+Test_Fold+'_V'+valid_Fold+'_NSGA2_trial'+str(i)+'.txt', 'w')
