...
if args.network == '2':
    ...
    valid_Fold = 'F4'
    trials = [1,2,3,4,5]
    Optimizers = ['COBYLA', 'DualAnn', 'GP', 'NSGA2']
    for op in range(len(Optimizers)):
        trialsDataFile = open(r'G:/Automated HybridLSTM (Skin) - Entropy/ReportsT_'+Test_Fold+'/Summary_all_methods/'+ Optimizers[op]+ '_T' +Test_Fold+'_V'+valid_Fold+'.txt', 'w')
        print (op)
        print('Optimizer: ', Optimizers[op])
        for i in trials:
            if op == 0:
                file = open(r'G:/Automated HybridLSTM (Skin) - Entropy/ReportsT_'+Test_Fold+'/COBYLA/ExcImgs_T'+Test_Fold+'_V'+valid_Fold+'_COBYLA_trial'+str(i)+'.txt', 'w')
            elif op == 1:
                file = open(r'G:/Automated HybridLSTM (Skin) - Entropy/ReportsT_'+Test_Fold+'/DualAnn/ExcImgs_T'+Test_Fold+'_V'+valid_Fold+'_DualAnn_trial'+str(i)+'.txt', 'w')
            elif op == 2:
                file = open(r'G:/Automated HybridLSTM (Skin) - Entropy/ReportsT_'+Test_Fold+'/GP/ExcImgs_T'+Test_Fold+'_V'+valid_Fold+'_GP_trial'+str(i)+'.txt', 'w')
            elif op == 3:
                file = open(r'G:/Automated HybridLSTM (Skin) - Entropy/ReportsT_'+Test_Fold+'/NSGA2/ExcImgs_T'+Test_Fold+'_V'+valid_Fold+'_NSGA2_trial'+str(i)+'.txt', 'w')
