...
if args.network == '2':
    ...
    valid_Fold = 'F4'
    trials = [1,2,3,4,5]
    #Optimizers = ['GP', 'Forest', 'Dummy', 'GBRT']
    Optimizers = ['COBYLA','DualAnn']
    for op in range(len(Optimizers)):
        print (op)
        print('Optimizer: ',Optimizers[op])
        for i in trials:
            """if op == 0:
                file = open(r'G:/Automated Deep spatial Fusion (skin)/ReportsT_'+Test_Fold+'/GP/ExcImgs_T'+Test_Fold+'_V'+valid_Fold+'_GP_trial'+str(i)+'.txt', 'w')
            elif op == 1:
                file = open(r'G:/Automated Deep spatial Fusion (skin)/ReportsT_'+Test_Fold+'/Forest/ExcImgs_T'+Test_Fold+'_V'+valid_Fold+'_Forest_trial'+str(i)+'.txt', 'w')
            elif op == 2:
                file = open(r'G:/Automated Deep spatial Fusion (skin)/ReportsT_'+Test_Fold+'/Dummy/ExcImgs_T'+Test_Fold+'_V'+valid_Fold+'_Dummy_trial'+str(i)+'.txt', 'w')
            elif op == 3:
                file = open(r'G:/Automated Deep spatial Fusion (skin)/ReportsT_'+Test_Fold+'/GBRT/ExcImgs_T'+Test_Fold+'_V'+valid_Fold+'_GBRT_trial'+str(i)+'.txt', 'w')
            """
            if op == 0:
                file = open(r'G:/Automated Deep spatial Fusion (skin)/ReportsT_'+Test_Fold+'/COBYLA/ExcImgs_T'+Test_Fold+'_V'+valid_Fold+'_COBYLA_trial'+str(i)+'.txt', 'w')
            elif op == 1:
                file = open(r'G:/Automated Deep spatial Fusion (skin)/ReportsT_'+Test_Fold+'/DualAnn/ExcImgs_T'+Test_Fold+'_V'+valid_Fold+'_DualAnn_trial'+str(i)+'.txt', 'w')
            ...
            """if op == 0:
                ...
                res = forest_minimize(f, space, n_calls=50)
            elif op == 2:
                res = dummy_minimize(f, space, n_calls=50)
            elif op == 3:
                res = gbrt_minimize(f, space, n_calls=50)
            """
