...
def Two_Stage_Output (p1, threshold, mode = 'test'):
    ...
    file.write('Deep Spatial Fusion CNN output:\n')
    file.write('--------------------------------------\n')
...
if args.network == '2':
    ...
    #file = open(r'G:/Automated Deep spatial Fusion (skin)/ReportsT_F4/Forest/BestBetas_ExcImgs_TF4_VF5_Forest.txt', 'w')
    file = open(r'G:/Automated Deep spatial Fusion (skin)/ReportsT_F5/Test_F5_all_methods.txt', 'w')
    
    opts = ['NSGA2', 'COBYLA', 'GBRT', 'Dummy', 'Forest', 'GP']
    Betas = [0.00726, 0.3225, 0.234364, 0.299356, 0.34726, 0.050985]
    op = 0
    for x in Betas:
        file.write('optimizer: '+str(opts[op]))
        file.write('\n')
